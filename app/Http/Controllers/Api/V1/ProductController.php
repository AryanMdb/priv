...
class ProductController extends BaseController
{
    ...
    public function getSubCategories(Request $request)
    {
        try {
            ...
            $subcategories = $subcategories->orderBy('order', 'asc')
                ->orderBy('id', 'desc')
                ->get();
        ...
        try {
            ...
            if ($category_charges->delivery_charge !== null) {
                ...
                foreach ($min_values as $index => $min_value) {
                    $max_value = $max_values[$index];
                    $charge = $delivery_charges[$index];
                    if ($cart->total_price >= $min_value && $cart->total_price <= $max_value) {
                        $delivery_charge = $charge;
                        break;
                    }
                }
    ...
    public function placeOrder(OrderRequest $request)
    {
        ...
        try {
            ...
            if (!$cart->location_id) {
                return response()->json(['error' => 'Location ID is missing for this cart'], 400);
            }
            ...
            if ($cart->coupon_id) {
                ...
                if (!$coupon) {
                    return $this->sendError([], 'Invalid coupon.', 400);
                }

                if ($coupon->expires_at < now()) {
                    return $this->sendError([], 'The applied coupon has expired.', 400);
                }
            }
            ...
            if (!empty($category->delivery_time)) {
                try {
                    $categoryDeliveryTime = Carbon::createFromFormat('H:i:s', $category->delivery_time)
                        ->setDate($currentTime->year, $currentTime->month, $currentTime->day);
                } catch (\Exception $e) {
                    $categoryDeliveryTime = null;
                }
            } else {
                $categoryDeliveryTime = null;
            }

            if ($categoryDeliveryTime) {
                if ($currentTime->greaterThan($categoryDeliveryTime)) {
                    $deliveryDate = $currentTime->copy()->addDay()->toDateString();
                } else {
            ...
            if ($cart->grant_total < 0) {
                return $this->sendError([], 'Invalid amount: The grand total cannot be negative.', 400);
            }
    ...
    public function applyCoupon(Request $request)
    {
        try {
            ...
            if ($total_price < $coupon->discount_value) {
                return response()->json([
                    'success' => false,
                    'message' => 'Your Total Amount is Small to Coupon',
                    'data' => null
                ], 404);
            }
            ...
            if ($coupon->status != 0 && Carbon::now()->lessThanOrEqualTo($coupon->expires_at)) {
                if ($request->coupon_code === $coupon->code) {
                    ...
                    if ($grant_total < 0) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Applying this coupon would result in a negative total, which is not allowed.',
                            'data' => null
                        ], 400);
                    }
                    ...
                    if ($total_price < $coupon->discount_value) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Your Total is Small to Coupon',
                            'data' => null
                        ], 404);
                    }
